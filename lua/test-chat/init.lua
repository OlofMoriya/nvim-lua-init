...
function filter_ansi_codes(strings)
...
function M.ask()
  ...
  stream_command_output_to_buffer({'/Users/olofmoriya/versioned/personal/chat/target/release/chat', '-c vim', '-m "' .. input .. '"'})
end
...
function M.replace()
    ...
    -- print("replace "..table.concat(start, '<').." finish "..table.concat(finish, "<"))
    vim.api.nvim_buf_set_lines(bufnbr, start[1]-1, finish[1], false, {""})

    -- print(table.concat(text, "."))
    stream_command_output_to_buffer({'/Users/olofmoriya/versioned/personal/chat/target/release/chat', '-c', 'vim', '-m','\'' .. table.concat(text, "\n") .. '\''})
end
